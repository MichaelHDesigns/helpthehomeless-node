...
To develop bitcore-node-helpthehomeless:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-helpthehomeless.git
...
cd bitcore-lib
npm install
cd ../bitcore-node-helpthehomeless
npm install
```
...
We now will setup symlinks in `bitcore-node-helpthehomeless` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-helpthehomeless
ln -s ~/bitcoind-rpc-helpthehomeless
```
...
cd devnode
mkdir node_modules
touch bitcore-node-helpthehomeless.json
touch package.json
```

Edit `bitcore-node-helpthehomeless.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
cd node_modules
ln -s ~/bitcore-lib
ln -s ~/bitcore-node-helpthehomeless
ln -s ~/insight-api
ln -s ~/insight-ui
```
...
```bash
../bitcore-node-helpthehomeless/bin/bitcore-node-helpthehomeless start
```